...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      const pageContent = idx => `Found ${results.length} result${results.length === 1 ? '' : 's'}\n\n${pages[idx]}`;

      try { await interaction.editReply({ content: pageContent(pageIdx), components: totalPages > 1 ? componentsFor('whohas', true, totalPages <= 1) : [] }); } catch (err) { debug.error('whohas initial editReply failed:', err); try { await interaction.followUp({ content: pageContent(pageIdx), components: totalPages > 1 ? componentsFor('whohas', true, totalPages <= 1) : [] }); } catch (e) { } }
      ...
      if (totalPages > 1) {
        ...
        collector.on('collect', async i => {
          ...
          await require('../utils/interaction').safeComponentUpdate(i, replyMsg, { content: pageContent(pageIdx), components: componentsFor('whohas', pageIdx === 0, pageIdx === totalPages - 1) });
        });
